...
| Quality control (v1.21.5+) | Stitch-time contradiction conflicts with source-safe evidence checks; additive `payload.execution_provenance` on typed findings/risks/decisions/verifications (explicit unknown cost/usage); acceptance criteria preserved through adapter prompts with per-criterion verification status |
...
Puppetmaster is **daily-driver beta software** (currently **v1.21.5**). The runtime contract is real, tests are automated, SQLite is the default backend, jobs fail closed, Cursor Agent MCP is live, and Claude Code + Codex have both been validated as full-edit adapters that emit patch artifacts. Hermes is wired as a fifth adapter (analyze + full-edit), with an end-to-end analyze run validated through the orchestrator (typed findings, CodeGraph context injection, stitched summary). The 1.21.x line adds dependency-aware reusable validation, collision-proof centralized run IDs, and deterministic quality control (contradiction conflicts, execution provenance, acceptance criteria) without breaking file/SQLite/MCP contracts. It is credible for supervised local engineering workflows. It is not yet a hosted multi-user production service.
