...
Puppetmaster is **daily-driver beta software** (currently **v1.21.13**). The runtime contract is real, tests are automated, SQLite is the default backend, jobs fail closed, Cursor Agent MCP is live, and Claude Code + Codex have both been validated as full-edit adapters that emit patch artifacts. Hermes is wired as a fifth adapter (analyze + full-edit), with an end-to-end analyze run validated through the orchestrator (typed findings, CodeGraph context injection, stitched summary). The 1.21.x line adds dependency-aware reusable validation, collision-proof centralized run IDs, deterministic quality control (contradiction conflicts, execution provenance, acceptance criteria, agentic criterion reporting), fail-fast model-backed auto-routing, Codex flat artifact normalization, Marionette-keyed Z.AI / MiniMax / NVIDIA NIM agentic providers, and passive provider rate-limit harvest without breaking file/SQLite/MCP contracts. It is credible for supervised local engineering workflows. It is not yet a hosted multi-user production service.
