...
| `agentic` | Keys-only analyze + full-edit via direct provider HTTP APIs (no external CLI) | tool-loop artifacts + PATCH on implement; tokens + cache reads + `price_job` / savings | any provider API key (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GEMINI_API_KEY`, `GOOGLE_API_KEY`, `OPENROUTER_API_KEY`, `OPENCODE_GO_API_KEY`) or AWS Bedrock IAM / `AWS_BEARER_TOKEN_BEDROCK` (`provider=bedrock`, Converse + ConverseStream + live catalog) |
...
Puppetmaster is **daily-driver beta software** (currently **v1.21.9**). The runtime contract is real, tests are automated, SQLite is the default backend, jobs fail closed, Cursor Agent MCP is live, and Claude Code + Codex have both been validated as full-edit adapters that emit patch artifacts. Hermes is wired as a fifth adapter (analyze + full-edit), with an end-to-end analyze run validated through the orchestrator (typed findings, CodeGraph context injection, stitched summary). The 1.21.x line adds dependency-aware reusable validation, collision-proof centralized run IDs, deterministic quality control (contradiction conflicts, execution provenance, acceptance criteria, agentic criterion reporting), and Codex flat artifact normalization without breaking file/SQLite/MCP contracts. It is credible for supervised local engineering workflows. It is not yet a hosted multi-user production service.
