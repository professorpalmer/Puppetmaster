...
Puppetmaster is **daily-driver beta software** (currently **v1.20.5**). The runtime contract is real, tests are automated, SQLite is the default backend, jobs fail closed, Cursor Agent MCP is live, and Claude Code + Codex have both been validated as full-edit adapters that emit patch artifacts. Hermes is wired as a fifth adapter (analyze + full-edit), with an end-to-end analyze run validated through the orchestrator (typed findings, CodeGraph context injection, stitched summary). The 1.20.x line adds a durable execution graph, constrained model allowlists with bounded same-adapter recovery, verified Bedrock invoke health before auto-routing, and a one-command `swarm` CLI twin of `start_cursor_swarm` for MCP-drop fallback. It is credible for supervised local engineering workflows. It is not yet a hosted multi-user production service.
