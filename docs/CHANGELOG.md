...
- **Per-model wire routing.** Chat models (DeepSeek V4, Kimi, GLM, MiMo, Hy3,
  Grok, …) use ``/chat/completions``; MiniMax/Qwen use ``/messages``; GPT 5.6
  Luna uses the stdlib OpenAI Responses transport (``/responses``, Bearer
  auth, no ChatGPT-only headers/metadata) — never silently posted to
  chat/completions.
- **Go quirks.** Flat-namespace normalization, reasoning dialects, and the
  MiMo Pro output ceiling — stdlib-only, no Marionette transport duplication.
