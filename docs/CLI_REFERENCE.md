...
python -m puppetmaster antigravity "Goal" --mode implement
...
Read-only, local, **numbers-only** — emits nothing over the network. Two measured cost pillars: **routing dollars saved** (each decision snapshots a frontier `baseline_cost_usd` at decision time; only `balanced`/`cheap` count, `quality`/`escalating` are shown as deliberate spend) and **CodeGraph exploration** (context tokens fed, measured; avoided directory-crawl tokens, a clearly-labeled estimate). Plus real-count lines that are never dollarized: **reliability** (tasks auto-recovered off a dead provider + tasks re-run for confidence), **$0 follow-up reads** (user-facing `show`/`artifacts`/`partial_summary`/`feed` reads served from durable state at zero model cost; recorded once per invocation at the command entry, never the store's internal reads, and the `feed --follow` long-poll is excluded so it can't inflate), and **working-set reuse skips** (analysis workers served from instruction+scope fingerprints; this is durable state, not provider KV cache). Tune the codegraph estimate with `PUPPETMASTER_EXPLORATION_BASELINE_TOKENS` / `PUPPETMASTER_EXPLORATION_PRICE_PER_MTOK`; disable the codegraph usage log with `PUPPETMASTER_CODEGRAPH_USAGE=0`, the reads log with `PUPPETMASTER_READS_USAGE=0`, and working-set skip metering with `PUPPETMASTER_WORKING_SET_USAGE=0`.
...
python -m puppetmaster cell-status [cell_id]     # path, inbox depth, hibernating, next_alarm
python -m puppetmaster cell-inspect <cell_id>    # plus inbox rows; file is sqlite3
python -m puppetmaster cell-tick                 # wake due alarms (interned poll)
