...
Puppetmaster turns the agent CLIs you already pay for — Cursor, Claude Code (Anthropic or AWS Bedrock), the OpenAI API, the Codex CLI, or Hermes — into an orchestrator. Or run it with no external CLI at all: point the built-in `agentic` adapter at any provider API key (OpenAI, Anthropic, Gemini, OpenRouter, **AWS Bedrock** IAM/BYOK) and it runs the whole tool-use loop itself — ideal for CI, containers, and headless servers. Bedrock uses the Converse API (and ConverseStream for live text/reasoning/tool deltas) with live account model discovery and the same prompt-cache / token / cost metering as other providers; automatic routing requires a verified runtime invoke, so stale AWS profiles cannot trap workers in repeated 403 failures. Either way it routes each task to the cheapest model that can handle it, runs workers as independent processes, and stores their output as typed SQLite artifacts, so follow-up reads cost zero tokens.
...
Daily-driver beta, currently at **v1.21.8**. Real runtime contract, automated tests, SQLite backend, fail-closed jobs, a live Cursor Agent MCP, validated full-edit adapters, constrained same-adapter model routing, and AWS Bedrock as a first-class agentic provider (Converse + ConverseStream) with verified-invoke routing health. Credible for supervised local engineering; not yet a hosted multi-user service. Full feature matrix in [FEATURES.md](https://github.com/professorpalmer/Puppetmaster/blob/main/docs/FEATURES.md). Versioned history in [CHANGELOG.md](https://github.com/professorpalmer/Puppetmaster/blob/main/docs/CHANGELOG.md).

**v1.21.8 highlights:** Public version markers and release documentation are synchronized with the current package release.

**v1.21.7 highlights:** Codex read-only workers retain flat finding, risk, and decision artifacts while rejecting ambiguous or malformed responses.

**v1.21.6 highlights:** Structured agentic `submit_findings` acceptance-criterion status; fail-closed task-scoped evidence normalization with non-raising verification fallback; agentic-only empty cached catalog unverified preflight while authoritative/credential failures still block.

**v1.21.5 highlights:** Deterministic contradiction conflicts with source-safe evidence checks; truthful per-artifact execution provenance (explicit unknown cost/usage); first-class acceptance criteria preserved through adapter prompts and verification artifacts — all additive to file/SQLite/MCP contracts.

**v1.20.8 highlights:** Agentic `--model` pins merge registry `payload_defaults` (provider/openrouter); analyze max-turns forces a last-chance `submit_findings` so Muse/OpenRouter smokes emit typed FINDINGs instead of false-green empty runs.
