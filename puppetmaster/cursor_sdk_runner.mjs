...
// Preserve parameters/variants so registry merge can stamp the UI default
// (e.g. grok-4.5 High+Fast) without inventing expanded alias ids.
if (input.mode === "list-models") {
  try {
    ...
    process.stdout.write(
      JSON.stringify({
        ...
        models: (models || []).map((m) => {
          ...
          if (Array.isArray(m.aliases) && m.aliases.length) {
    ...
    // Soft-complete: process.exit() after Cursor.models.list() races libuv
    // handle teardown on Windows ("UV_HANDLE_CLOSING" assertion).
    process.exitCode = 0;
  } catch (error) {
    console.error(
  ...
  try {
    ...
    let output = "";
    try {
      const modelSelection = { id: input.model || "default" };
      // Registry discovery stamps the UI default variant as payload.params
      // (e.g. effort=high, fast=true for grok-4.5). Forward them unchanged.
      if (Array.isArray(input.params) && input.params.length) {
        modelSelection.params = input.params;
      }
