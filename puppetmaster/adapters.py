...
class CursorAdapter:
    ...
    def run(self, task: Task, goal: str, worker_id: str) -> list[Artifact]:
        prompt = task.payload.get("prompt") or task.instruction
        cwd = task.payload.get("cwd")
...
ADAPTERS: dict[str, WorkerAdapter] = {
    ...
    "claude-code": UnconfiguredProviderAdapter("claude-code", "Claude Code"),
...
ADAPTER_INFO = [
    ...
    AdapterInfo(
        name="claude-code",
        status="stub",
        description="Provider-neutral placeholder for a future Claude Code adapter.",
